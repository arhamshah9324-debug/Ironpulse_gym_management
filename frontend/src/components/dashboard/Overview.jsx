      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <StatCard icon={<RefreshCw size={20} />} value={stats.active_subscriptions} textColor="text-black" title="Active Subs" iconBg="bg-green-100" iconColor="text-green-600" />
            <StatCard icon={<CheckSquare size={20} />} value={stats.today_attendance} textColor="text-black" title="Today Ins" iconBg="bg-yellow-100" iconColor="text-yellow-600" />
            <StatCard icon={<AlertCircle size={20} />} value={stats.expiring_soon} textColor="text-black" title="Expiring" iconBg="bg-red-100" iconColor="text-red-600" />
            <StatCard icon={<Dumbbell size={20} />} value={stats.total_trainers} textColor="text-black" title="Trainers" iconBg="bg-purple-100" iconColor="text-purple-600" />
          </div>
        ) : null}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 card p-6">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={stats?.attendance_trend || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                >
                  {(stats?.attendance_trend || []).map((entry, index) => (
          ...
          <div className="space-y-6 flex flex-col">
            ...
            <div className="card p-6 flex-1 drop-shadow-sm">
              ...
              <div className="space-y-4">
                {(stats?.recent_activity && stats.recent_activity.length > 0) ? stats.recent_activity.map((activity, i) => (
                  <div key={activity.id} className="flex items-start gap-3 group">
                    <div className="relative mt-1">
                      <div className={`w-2 h-2 rounded-full z-10 relative ${activity.type === 'payment' ? 'bg-green-500' :
                        activity.type === 'trainer' ? 'bg-purple-500' : 'bg-[var(--accent-warm)]'
                        }`}></div>
                    ...
                    <div>
                      ...
                      <div className="text-[10px] uppercase tracking-wider font-bold text-[var(--text-muted)] mt-1">{getTimeAgo(activity.time)}</div>
                    </div>
                  </div>
                )) : (
                  <div className="text-xs text-gray-400 font-medium italic py-4">No recent activity found.</div>
                )}
              </div>
            </div>
          </div>
